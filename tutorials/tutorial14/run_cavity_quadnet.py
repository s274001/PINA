...
logging.basicConfig(filename='cavity_sensitivity_log_test.txt',level=logging.INFO,format='%(message)s')
class LogCallback(Callback):
    def on_train_start(self, trainer, pl_module):
        self.tstart = time.time()
    def on_train_end(self,trainer,pl_module):
        self.tend = time.time()
        logging.info(f'reddim={pl_module.neural_net["reduction_network"].rank}, v_num={pl_module.logger.version}, loss_corr={trainer.callback_metrics["loss_corr"].item()}, train_time={self.tend-self.tstart}')
...
def main(arguments=None):
    ...
    cavity = CavityProblem(field,reddim,subset=None, train_size=args.train, test_size=100, device='gpu')
    # cavity.gpu()
    # print(cavity.indices)
    # if not args.load:
    #     cavity_ref = CavityProblem(field,reddim)
    #     fig_,ax_ = plt.subplots(figsize=(10,10))
    #     ax_.set_aspect('equal')
    #     ax_.scatter(cavity.coords['x'].detach().numpy(),cavity.coords['y'].detach().numpy(), c='k')
    #     ax_.triplot(cavity_ref.data.triang,c='grey',zorder=-1)
    #     fig_.savefig('img/quadnet_training_points')
    #     # plt.show()
    #     del fig_,ax_,cavity_ref
    #     plt.gcf().clear()
    ...
    if not args.load:
        ...
        rom = CorrectedROM(problem = problem,
                           ...
                           # scheduler_kwargs={'total_iters':200,
                           #                   'factor':0.1}
                           )
        ...
        early = EarlyStopping(monitor='loss_corr',patience=20000,stopping_threshold=1e-2, check_on_train_epoch_end=True)
        switch_opt = SwitchOptimizer(torch.optim.LBFGS,{'lr':1e-2},1000)
        trainer = Trainer(solver=rom, 
                          max_epochs=epochs, 
                          accelerator='gpu',
                          default_root_dir=args.load, 
                          callbacks = [MetricTracker(),LogCallback(),early,switch_opt],
                          batch_size=args.train//num_batches,
                          )
        trainer.train()
        
        pl = Plotter()
        pl.plot_loss(trainer=trainer,label='loss_corr',logy=True,filename=f'quadnet/loss_{reddim}_{args.train}')
        #plt.show()
...
if __name__ == "__main__":
    ...
    # for r in dimensions:
    #     for size in sizes:
    #         args = ['--epochs', '40000', '--reddim', f'{r}', '--train', f'{size}']
    #
    #         main(args)
    main(sys.argv[1:])
