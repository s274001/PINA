...
class TimeDependentProblem(AbstractProblem):
    ...
    :Example:
        ...
        >>>
        >>>     conditions = {
        >>>         't0': Condition(location=CartesianDomain({'x': [0, 3], 't':0}), equation=Equation(initial_condition)),
        >>>         'gamma1': Condition(location=CartesianDomain({'x':0, 't':[0, 1]}), equation=FixedValue(0.)),
        >>>         'gamma2': Condition(location=CartesianDomain({'x':3, 't':[0, 1]}), equation=FixedValue(0.)),
        >>>         'D': Condition(CartesianDomain({'x': [0, 3], 't':[0, 1]}), equation=Equation(wave_equation))}
